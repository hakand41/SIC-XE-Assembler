...
def run_assembler():
    ...
    status_label.config(text="Compiling Sic_Assembler.cpp...")
    root.update_idletasks()  # Refresh the UI

    compile_process = subprocess.run(['g++', '-o', 'Sic_Assembler', 'Sic_Assembler.cpp'])
    if compile_process.returncode != 0:
        messagebox.showerror("Error", "Failed to compile Sic_Assembler.cpp")
        status_label.config(text="Compilation failed.")
        return
    
    status_label.config(text="Running Sic_Assembler...")
    root.update_idletasks()  # Refresh the UI

    run_process = subprocess.run(['./Sic_Assembler'])
    if run_process.returncode != 0:
        messagebox.showerror("Error", "Failed to run Sic_Assembler")
